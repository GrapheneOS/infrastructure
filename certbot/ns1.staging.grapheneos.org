certbot certonly --webroot --webroot-path /srv/certbot --max-log-backups 0 --no-eff-email \
    --key-type ecdsa --reuse-key --required-profile shortlived \
    --deploy-hook "systemctl reload nginx; rsync -acv --copy-links --delete --chmod=D750,F640 --chown root:dnsdist /etc/letsencrypt/live/ /etc/letsencrypt/dnsdist/; dnsdist -c -e 'reloadAllCertificates()'" \
    --cert-name ns1.staging.grapheneos.org \
    -d ns1.staging.grapheneos.org \
    -d ns2.staging.grapheneos.org \
    -d ns1.staging.attestation.app \
    -d ns2.staging.attestation.app
